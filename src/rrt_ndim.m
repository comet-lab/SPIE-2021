...
while true
    ...
    % check if tip is within view of camera if enabled
    Ptip = robot.pose(:,end);
    view = robot.endo.tipInBounds(Ptip);
    ...
    [inter,dists, ~, ~, ~] = TriangleRayIntersection(...
        Pcam, uray, v1, v2, v3);         % check intersections between ray and faces
    flagged = sum(inter) > 0;           
